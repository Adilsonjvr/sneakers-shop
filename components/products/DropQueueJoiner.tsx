...
export function DropQueueJoiner({ dropId }: DropQueueJoinerProps) {
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold uppercase tracking-[0.3em] text-brand">Drop Live</p>
      <p className="text-sm text-white/80">
        Reserva o teu lugar na fila. Receberás notificação assim que liberarmos o checkout.
      </p>
      ...
      {status && (
        <p className="mt-3 text-sm text-white/70">
          ...
          {status.position && ` · posição ${status.position}`}
